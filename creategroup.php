...
?></small>
				...
				<div class="sub-header">
					<div class="d-flex align-items-center flex-wrap mr-auto">
						<h5 class="dashboard_bar">Form Group</h5>
					</div>
			...
			<div class="deznav-scroll">
				...
				<ul class="metismenu" id="menu">
					...
					<ul aria-expanded="false">
						<li><a href="CheckTeammate.php">See Teammate Information</a></li>
			...
			<div class="container-fluid">
                <div class="form-group row">
                    ...
                    <form method="post" action="sendrequest.php" >
                      <label for="ex1">GroupName</label>
                      <input class="form-control" name="groupName" id="groupName" type="text">
                    </div>
                    </div>
                <div class="mb-3">
                    <label for="exampleFormControlInput1" class="form-label">Student Lists</label>
                </div>
                ...
                <table class="table" id="table">
                    ...
                    $conn = mysqli_connect($servername, $username, $password, $dbname) or die("Database Connection failed: " . mysqli_connect_error());
                    $loginId=$_SESSION["LoginUserID"];
					$sql = "SELECT * FROM user WHERE role=0 AND user_id != $loginId ";
					$rs = mysqli_query($conn, $sql);
                    $number=1;
					...
					while ($rc = mysqli_fetch_assoc($rs)) {
                        ...
                        $user_id=$rc['user_id'];
                        $sql2="SELECT student_id from student_in_group WHERE student_id = $user_id" ;
                        $rd = mysqli_query($conn, $sql2);
                        $result=mysqli_num_rows($rd);
                        if($result<=0){
                        echo "<tr> <th scope='row'>$number</th>
                        <td>$user_name</td>
                        <td><input class='form-check-input' type='checkbox' name='choose[]' value='$user_id' id='flexCheckDefault'></td></tr>
                        ";
                        $number=$number+1;
                    }
                    }?>
