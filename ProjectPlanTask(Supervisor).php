...
if ($_SESSION["LoginUserRole"] == 0) {
    ...
    echo "Admin";
} else if ($_SESSION["LoginUserRole1"]== 3){
...
?></small>
				...
				<div class="sub-header">
                    ...
                    <li class="breadcrumb-item"><a>Teams</a></li>
                        ...
                        <li class="breadcrumb-item active"><a href="ProjectPlan(Supervisor).php">Task Details </a></li>
					</div>
	...
	while($rc = mysqli_fetch_assoc($rs)){
		...
		<div class="modal fade" id="edit' . $rc['task_id'] . '">
            <div class="modal-dialog modal-dialog-centered" role="document">
				...
				<textarea id="Suggestion" class="form-control" name="Suggestion" rows="5" cols="50" > '.$rc['suggestion'].'</textarea>
                        
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger light"
                            data-dismiss="modal">Close</button>
                        <button type="submit" name="sb' . $rc['task_id'] . '"    id="sb' . $rc['task_id'] . '" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
		...
		echo '<form method="post"><tr>
			...
			<td>
				<div class="d-flex">
					...
					<a href="#" data-toggle="modal" name="upload'.$rc['task_id'] .'" id="upload'.$rc['task_id'] .'" data-target="#upload'.$rc['task_id'] .'" class="btn btn-dark shadow btn-xs sharp mr-1"><i class="fa fa-download"></i></a>
					<a href="#" data-toggle="modal"  name="edit'.$rc['task_id'] .'"   id="edit'.$rc['task_id'] .'"     data-target="#edit'.$rc['task_id'] .'" class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fa fa-tag"></i></a>
				</div>
			</td>
		</tr></form>';
...
if (isset($_POST[$button_name])) {
  ...
  if (isset($_POST[$Editbutton_name])) {
	...
	$sqlEdit = "UPDATE task SET suggestion = '".$Suggestion."' WHERE task_id = ".$rc['task_id'];
    ...
    $row1 = mysqli_fetch_array($rs1);
		...
		$detail= $row1['task_description'];
			...
			<div class="modal-body">
				...
				<textarea id="Description" class="form-control" name="Description" rows="5" cols="50"  readonly>'.$detail.'</textarea>
